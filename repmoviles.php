...
if ($agregar) {  
				...
				$ingreso=mysql_query("insert into $tabla (cliente, marca, modelo, imei, extras, averia, servtec, bateria, cargador, presupuesto, total, aprobado, fecha, estado, tienda) values ('$cliente', '$marca', '$modelo', '$imei', '$extras', '$averia', '$servtec', '$bateria', '$cargador', '$presupuesto', '$total', '$aprobado', '$lafecha', 'abierto', '$sucursal')");
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
     ...
     <span class="texto_chico">No es necesario incluir RM<? if ($sucursal=="Arrecife") { echo "A"; } if ($sucursal=="Centro") { echo "C"; } if ($sucursal=="Tias") { echo "T"; }  ?>
     </span> 
       <input type="text" name="buscado" id="buscado"  style="width:200px" <? if ($buscar) { ?> value="<? echo $buscado; ?>"<? } ?>/> 
       &nbsp;
       <input type="submit" name="buscar" id="buscar" value="Buscar" />
       <? if ($buscar) { ?>&nbsp;<a href='repmoviles.php'>&laquo; Volver</a><? } ?>
      </form>
      </div>
          ...
          <br />
            ...
            <table width="600" border="0" cellspacing="3" cellpadding="0">
              <tr>
                ...
                <td><input style="width:98%" type="text" name="marca" id="marca" /></td>
              </tr>
              <tr>
                <td>Modelo</td>
              ...
              <tr>
                <td>IMEI</td>
                <td><input style="width:98%" type="text" name="imei" id="imei" /></td>
              </tr>
              <tr>
                <td>Deja cargador</td>
              ...
              <tr>
                <td nowrap="nowrap">Service Oficial</td>
      ...
      <br />
      <div style="float:right; clear:both"><? if ($cls!="cerrados") { ?><a href="repmoviles.php?cls=cerrados">&raquo; ver ordenes finalizadas</a><? } else { ?><a href="repmoviles.php">&raquo; ver ordenes pendientes</a><? } ?></div>
      <? 
	   ...
	   if ($buscar) { 
						...
						$filtro.=" and id_repmovil like '$buscadofiltrado'"; 
						} 
	   ...
	   if ($sucursal=="Centro") { $tabla="repmovil_centro"; } 
		...
		$orden=" order by id_repmovil desc"; 
      ...
      <table id="repmoviles" style="border:1px solid #E5E5E5; margin-top:10px" width="930" border="0" cellspacing="3" cellpadding="0">
        <thead>
        <tr >
		...
		while ($saco=mysql_fetch_array($busco)) { 
		$link=" style='cursor:pointer; border-bottom:1px dotted #666;' onclick=\"location.href='repmoviles_editar.php?id=$saco[0]'\""; ?>
        <tr>
          <td <? echo $link; ?>>RM<? if ($saco[tienda]=="Tias") {  echo "T"; } if ($saco[tienda]=="Centro") {  echo "C"; }  if ($saco[tienda]=="Arrecife") {  echo "A"; }  printf("%05d",  $saco[0]); $saco[0]; ?></td>
          ...
          <td <? echo $link; ?>><? echo "$saco[marca] $saco[modelo]"; ?></td>
          <td align="center" <? echo $link; ?>><? if ($saco[presupuesto]=="0.00") { echo "&nbsp;"; } else { echo "Si"; } ?></td>
          <td align="center" <? echo $link; ?>><? if ($saco[sms]=="") { echo "&nbsp;"; } else { echo "Si"; } ?></td>
          <td align="center" <? echo $link; ?>><? if ($saco[aprobado]=="si") { echo "Si"; } else { echo "&nbsp;"; }  ?></td>
          <td align="center" <? echo $link; ?>><? if ($saco[sms2]=="") { echo "&nbsp;"; } else { echo "Si"; } ?></td>
        </tr>
        <? } ?>
      </tbody>
      </table>
