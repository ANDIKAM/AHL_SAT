...
if ($agregar) {  
				...
				$ingreso=mysql_query("update $tabla set cliente='$cliente', tipoequipo='$tipoequipo', averia='$averia', modelo='$modelo', nserie='$nserie', danos='$danos', cargador='$cargador', bateria='$bateria', contrasena='$contrasena', extras='$extras', trabajo='$trabajo', presupuesto='$presupuesto', aprobado='$aprobado', total='$total', solucion='$solucion' where id_computadoras='$id'");
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
      ...
      <span class="titulo">Orden de reparación: RP<? if ($saco[tienda]=="Tias") {  echo "T"; } if ($saco[tienda]=="Centro") {  echo "C"; }  if ($saco[tienda]=="Arrecife") {  echo "A"; } printf("%05d",  $saco[0]); $saco[0]; ?>
      </span>
      ...
      <div style="float:right; margin-top:17px">
        ...
        <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Hoja de reparación</span><br />
            <br />
            <a href="computadoras_orden.php?id=<? echo $id; ?>">Imprimir hoja de reparación</a><br />
        </div>
      ...
      <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Avisos</span><br />
            <br />
            <? if ($saco[sms]!="si") { ?><a href="computadoras_aviso_presupuesto.php?id=<? echo $id; ?>">Enviar aviso de <strong>Presupuesto</strong></a><? } else { echo "Aviso de presupuesto enviado !"; } ?>
            <div style="height:7px"></div>
            <? if ($saco[sms2]!="si") { ?><a href="computadoras_aviso_terminado.php?id=<? echo $id; ?>">Enviar aviso de <strong>reparacion lista</strong></a><? } else { echo "Aviso de finalización enviado !"; } ?><br />
        </div>
        ...
        <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Facturación</span><br />
            <br />
            <a href="computadoras_facturar.php?cls=1&tipo=rp&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>&mdo=<? echo $saco[presupuesto]; ?>">Facturar reparación</a>
            <div style="height:7px"></div>
            <a href="computadoras_facturar.php?cls=2&tipo=rp&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>">Facturar Presupuesto 12 &#8364;</a>
             <div style="height:7px"></div>
            <a href="computadoras_facturar.php?cls=3&tipo=rp&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>">Cerrar sin facturar</a><br />
        </div>
        <? } ?>
            ...
            <table width="600" border="0" cellspacing="3" cellpadding="0">
              ...
              <tr>
                ...
                <td><div id="loscomponentes"><? 
						...
						$bcom=mysql_query("select * from $tablacomp where clase='rp' and id='$id'");
                ...
                <td width="20" bgcolor="#FFFFCC">&nbsp;</td>
										...
										while ($scom=mysql_fetch_array($bcom)) { $totalcomp=$totalcomp+$scom[total];
																				?><tr><td><? echo $scom[unidades]; ?></td><td><? echo utf8_encode($scom[descripcion]); ?></td><td nowrap="nowrap">&#8364; <? echo $scom[precio]; ?></td>
                  
                  <td align="right" nowrap="nowrap">&#8364; <? echo $scom[total]; ?></td>
                  <td align="center"><a href="componentes_eliminar.php?clase=rp&id=<? echo $id; ?>&idcom=<? echo $scom[0]; ?>">X</a></td>
                <?
																				}
          ...
          <?
                                        ...
                                        <div id="detallecomp" style="display:<? echo "none"; ?>; margin-top:10px; border:1px dotted #333">
                                          <table width="100%" border="0" cellspacing="3" cellpadding="0">
                                            ...
                                            <tr>
                                              <td>&nbsp;</td>
                                              <td><input type="button" name="button" id="button" value="Agregar componente" onClick="cargar_componente_rp(document.getElementById('cant').value,document.getElementById('desc').value,document.getElementById('precio').value,'<? echo $id; ?>','<? echo rand(111111,999999); ?>');mostrar('agregarcomp'); ocultar('detallecomp');document.getElementById('cant').value='';document.getElementById('desc').value='';document.getElementById('precio').value=''" /> 
