...
if ($agregar) {  
				...
				$ingreso=mysql_query("update $tabla set cliente='$cliente', marca='$marca', modelo='$modelo', imei='$imei', extras='$extras', averia='$averia', servtec='$servtec', bateria='$bateria', cargador='$cargador', presupuesto='$presupuesto', total='$total', aprobado='$aprobado', solucion='$solucion', trabajo='$trabajo' where id_repmovil='$id'");
				
				?><script>location.href='repmoviles.php?ssid=<? echo rand(111111,999999); ?>'</script><? }
 ?>
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
      ...
      <span class="titulo">Orden de reparación: RM<? if ($saco[tienda]=="Tias") {  echo "T"; } if ($saco[tienda]=="Centro") {  echo "C"; }   if ($saco[tienda]=="Arrecife") {  echo "A"; } printf("%05d",  $saco[0]); $saco[0]; ?>
      </span>
      ...
      <div style="float:right; margin-top:17px">
        ...
        <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Hoja de reparación</span><br />
            <br />
            <a href="repmoviles_orden.php?id=<? echo $id; ?>">Imprimir hoja de reparación</a><br />
        </div>
      ...
      <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Avisos</span><br />
            <br />
            <? if ($saco[sms]!="si") { ?><a href="repmoviles_aviso_presupuesto.php?id=<? echo $id; ?>">Enviar aviso de <strong>Presupuesto</strong></a><? } else { echo "Aviso de presupuesto enviado !"; } ?>
            <div style="height:7px"></div>
            <? if ($saco[sms2]!="si") { ?><a href="repmoviles_aviso_terminado.php?id=<? echo $id; ?>">Enviar aviso de <strong>reparacion lista</strong></a><? } else { echo "Aviso de finalización enviado !"; }?><br />
        </div>
        ...
        <div align="center" style=" margin-right:20px; margin-top:20px; width:220px; padding:10px; border:1px dotted #999999; background-color: #FFFFCC"><span class="titulo">Facturación</span><br />
            <br />
            <a href="repmoviles_facturar.php?cls=1&tipo=rm&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>&mdo=<? echo $saco[presupuesto]; ?>">Facturar reparación</a>
            <div style="height:7px"></div>
            <a href="repmoviles_facturar.php?cls=2&tipo=rm&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>">Facturar Presupuesto 12 &#8364;</a>
             <div style="height:7px"></div>
            <a href="repmoviles_facturar.php?cls=3&tipo=rm&idcl=<? echo $saco[cliente]; ?>&id=<? echo $id; ?>">Cerrar sin facturar</a><br />
        </div>
        ...
        <form id="form1" name="form1" method="post" action="#SELF">
          ...
          <br />
            ...
            <table width="600" border="0" cellspacing="3" cellpadding="0">
              ...
              <tr>
                <td>Service oficial</td>
                <td><select name="servtec" id="servtec">
                  <option <? if ($saco[servtec]=="si") { ?>selected="selected"<? } ?>>si</option>
                  <option <? if ($saco[servtec]=="no") { ?>selected="selected"<? } ?>>no</option>
                </select></td>
              </tr>
              <tr>
                <td>Avería</td>
              ...
              <tr>
				...
				if ($sucursal=="Arrecife") { $tablacomp="componentes_arrecife"; }
						...
						$bcom=mysql_query("select * from $tablacomp where clase='rm' and id='$id'");
          ...
          <?
                                        ...
                                        <div id="detallecomp" style="display:<? echo "none"; ?>; margin-top:10px; border:1px dotted #333">
                                          <table width="100%" border="0" cellspacing="3" cellpadding="0">
                                            ...
                                            <tr>
                                              <td>&nbsp;</td>
                                              <td><input type="button" name="button" id="button" value="Agregar componente" onClick="cargar_componente_rm(document.getElementById('cant').value,document.getElementById('desc').value,document.getElementById('precio').value,'<? echo $id; ?>','<? echo rand(111111,999999); ?>');mostrar('agregarcomp'); ocultar('detallecomp');document.getElementById('cant').value='';document.getElementById('desc').value='';document.getElementById('precio').value=''" /> 
