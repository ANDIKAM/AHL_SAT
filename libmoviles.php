...
if ($agregar) {  
				...
				$ingreso=mysql_query("insert into $tabla (marca, modelo, imei, extras, cliente, aprobado, total, fecha, estado, tienda, cargador, bateria, presupuesto) values ('$marca', '$modelo', '$imei', '$extras', '$cliente', '$aprobado', '$total', '$lafecha', '$estado', '$sucursal', '$cargador', '$bateria', '$presupuesto')");
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
     ...
     <span class="texto_chico">No es necesario incluir LM<? if ($sucursal=="Arrecife") { echo "A"; } if ($sucursal=="Centro") { echo "C"; } if ($sucursal=="Tias") { echo "T"; }  ?>
     </span> 
       <input type="text" name="buscado" id="buscado"  style="width:200px" <? if ($buscar) { ?> value="<? echo $buscado; ?>"<? } ?>/> 
       &nbsp;
       <input type="submit" name="buscar" id="buscar" value="Buscar" />
       <? if ($buscar) { ?>&nbsp;<a href='libmoviles.php'>&laquo; Volver</a><? } ?>
      </form>
      </div>
      ...
      <div style="clear:both;float:right"><? if ($cls!="cerrados") { ?><a href="libmoviles.php?cls=cerrados">&raquo; ver ordenes finalizadas</a><? } else { ?><a href="libmoviles.php">&raquo; ver ordenes pendientes</a><? } ?></div>
      <? 
	  	if ($sucursal=="Arrecife") { $tabla="libmovil_arrecife"; }
	    if ($sucursal=="Tias") { $tabla="libmovil_tias"; } 
		if ($sucursal=="Centro") { $tabla="libmovil_centro"; } 
				
	  	 if ($cls=="") { $filtro.="  where estado='' "; } 
		 if ($cls=="cerrados") { $filtro.="  where estado='finalizado' "; } 
		  ...
		  if ($buscar) { 
						...
						$filtro.=" and id_libmovil like '$buscadofiltrado'"; 
						} 
		 
		 
		 
	 	$orden=" order by id_libmovil desc"; 
      ...
      <table id="libmoviles" style="border:1px solid #E5E5E5; margin-top:10px" width="930" border="0" cellspacing="3" cellpadding="0">
        <thead>
        <tr>
		...
		while ($saco=mysql_fetch_array($busco)) { 
		$link=" style='cursor:pointer; border-bottom:1px dotted #666;' onclick=\"location.href='libmoviles_editar.php?id=$saco[0]'\""; ?>
        <tr>
          <td <? echo $link; ?>>LM<? if ($saco[tienda]=="Tias") {  echo "T"; }  if ($saco[tienda]=="Centro") {  echo "C"; }  if ($saco[tienda]=="Arrecife") {  echo "A"; }  printf("%05d",  $saco[0]); $saco[0]; ?></td>
          ...
          <td align="center" <? echo $link; ?>><? if ($saco[presupuesto]!="0.00") { echo "Si"; } else { echo "No"; } ?></td>
