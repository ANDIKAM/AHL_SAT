...
if ($class=="new") {    $creo=mysql_query("insert into clientes_presupuesto (empresa, sucursal) values ('$id','$sucursal')");
						$b=mysql_query("select * from clientes_presupuesto order by id_clientes_presupuesto desc"); $s=mysql_fetch_array($b); 
						?><script>location.href='clientes_nuevo_presupuesto.php?id=<? echo $id; ?>&id_presupuesto=<? echo $s[0]; ?>'</script><? } 
...
if ($agregar) { if ($descuento=="" or $descuento=="0") { $total=$unidades*$precio; }  else {
				...
				$ingreso=mysql_query("insert into clientes_presupuesto_detalle (unidades, concepto, precio, total, descuento, presupuesto) values ('$unidades','$concepto','$precio','$total', '$descuento', '$id_presupuesto')");
				?><script>location.href='clientes_nuevo_presupuesto.php?id=<? echo $id; ?>&id_presupuesto=<? echo $id_presupuesto; ?>'</script><? }
 

if ($guardar_comentarios) { $actualizo=mysql_query("update clientes_presupuesto set comentario='$comentarios' where id_clientes_presupuesto='$id_presupuesto'"); 

			?><script>location.href='clientes_nuevo_presupuesto.php?id=<? echo $id; ?>&id_presupuesto=<? echo $id_presupuesto; ?>'</script><? } 



if ($enviar=="92346124") {	$b=mysql_query("select * from clientes where id_clientes='$id'"); 
...
Para dejar de recibir nuestra publicidad, es suficiente con indicarlo a cualquiera de nuestros comerciales o enviando un e-mail a: soporte@ahlinformatica.com asunto baja.";
							...
							require_once 'class.phpmailer.php';
							  ...
							  $actualizo=mysql_query("update clientes_presupuesto set enviado='si', fecha='$lafechadehoy', veces='$veces' where id_clientes_presupuesto='$id_presupuesto'");
							?><script>location.href='clientes_editar.php?id=<? echo $id; ?>&enviado=true'</script><? }
...
function confirmar_enviar() { if (confirm('Seguro que desea enviar el presupuesto')) { location.href='clientes_nuevo_presupuesto.php?id=<? echo $id; ?>&id_presupuesto=<? echo $id_presupuesto; ?>&enviar=92346124'} }
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
      <h1><a href="empresas_lista.php">Empresas</a> &raquo; <? $b=mysql_query("select * from clientes where id_clientes='$id'"); $s=mysql_fetch_array($b);  ?><a href='empresas_editar.php?id=<? echo $id; ?>'><? echo "$s[apellido] $s[nombre]"; ?></a> &raquo; Nuevo presupuesto</h1>
...
<table width="800" border="0" cellspacing="0" cellpadding="4" style="border:1px solid #666">
  <tr <? echo $estilo; ?>>
        ...
        <tr>
          ...
          <td  align="center" <? echo $estilo; ?>><a href="clientes_presupuesto_eliminar.php?id=<? echo $id; ?>&id_art=<? echo $scom[0]; ?>&id_presupuesto=<? echo $id_presupuesto; ?>">X</a></td>
        </tr>
        <? } ?>
        <tr >
