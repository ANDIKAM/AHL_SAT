...
if ($enviar) {	if ($sucursal=="Arrecife") { $tabla="libmovil_arrecife"; }
				...
				$mensaje="El presupuesto de su liberación está terminado, para continuar debe aceptarlo desde la siguiente dirección: http://sat.ahlinformatica.com/presupuesto";
				$mensaje=utf8_decode($mensaje);
				$asunto="Presupuesto de liberacion";
				...
				if ($s[movil1]!="") { 
										...
										$txt = "El presupuesto de su reparacion esta terminado, puede verificarlo en http://sat.ahlinformatica.com/clientes";
                                                                                $cod="LM".($saco[tienda]=="Tias"?"T":($saco[tienda]=="Centro"?"C":($saco[tienda]=="Arrecife"?"A":""))).str_pad($saco[0], 5, "0", STR_PAD_LEFT);
                                                                                $txt2="El presupuesto de su liberación $cod está terminado, tiene un costo de €$saco[presupuesto], para continuar la reparación debe aceptarlo desde la siguiente dirección: http://sat.ahlinformatica.com/presupuesto";
										$dst = "+34".$s[movil1];
				...
				$actualizo=mysql_query("update $tabla set sms='si' where id_libmovil='$id'");
