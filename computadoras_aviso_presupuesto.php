...
if ($enviar) {   if ($sucursal=="Arrecife") { $tabla="computadoras_arrecife"; }
				...
				if ($s[movil1]!="") {
										...
										$xid = 1;
                                                                                $cod="RP".($saco[tienda]=="Tias"?"T":($saco[tienda]=="Centro"?"C":($saco[tienda]=="Arrecife"?"A":""))).str_pad($saco[0], 5, "0", STR_PAD_LEFT);
										...
										//$send->sendOneSMS($xid, $txt, $dst); 
                                                                                ...
                                                                                //$wsa->sendMessage(array('573016403475'), 1, $txt2);
									}
									
				
				
				
				$actualizo=mysql_query("update $tabla set sms='si' where id_computadoras='$id'");
				
				?><script>location.href='computadoras_editar.php?id=<? echo $id; ?>'</script><?
