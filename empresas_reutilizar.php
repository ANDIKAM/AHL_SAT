...
if ($empresa_destino!="") {  $bp=mysql_query("select * from empresa_presupuesto where id_empresa_presupuesto='$id_presupuesto'"); $sp=mysql_fetch_array($bp); $comentarios=$sp[comentario];
							$busco=mysql_query("select * from empresa_presupuesto where enviado!='si' and empresa='$empresa_destino'");
							if ($saco=mysql_fetch_array($busco)) { $encontrado=$saco[0];
																  $elimino=mysql_query("delete from empresa_presupuesto_detalle where presupuesto='$encontrado'");
																 }
																 $creo=mysql_query("insert into empresa_presupuesto (empresa, sucursal, comentario) values ('$empresa_destino','$sucursal', '$comentarios')");
																 ...
																 while ($scom=mysql_fetch_array($bcom)) {	$unidades=$scom[unidades]; $concepto=$scom[concepto]; $precio=$scom[precio]; $total=$scom[total]; $descuento=$scom[descuento]; $elpresupuesto=$s[0];
																	 										$ingreso=mysql_query("insert into empresa_presupuesto_detalle (unidades, concepto, precio, total, descuento, presupuesto) values ('$unidades','$concepto','$precio','$total', '$descuento', '$elpresupuesto')");
																										} 
																 
								?><script>location.href='empresas_nuevo_presupuesto.php?id=<? echo $empresa_destino; ?>&id_presupuesto=<? echo $elpresupuesto; ?>'</script><?
							 } 
...
$busco=mysql_query("select * from empresa_presupuesto where id_empresa_presupuesto='$id_presupuesto'");
$saco=mysql_fetch_array($busco);

$b=mysql_query("select * from $tabla_empresas where id_empresa='$saco[empresa]'");
...
<table width="979" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <div style="margin-left:20px; margin-right:20px">
      ...
      <h1>Empresas &raquo; Reutilizar presupuesto</h1>
      Seleccione la empresa a la que desea enviarle el presupuesto<br />
      <br />
      Empresa original: <strong><a href='empresas_reutilizar.php?id_presupuesto=<? echo $id_presupuesto; ?>&empresa_destino=<? echo $s[0]; ?>'><? echo $s[nombre]; ?></a></strong>
<br />
      <br />
      <table style="border:1px solid #E5E5E5" width="930" border="0" cellspacing="3" cellpadding="0">
		...
		$filtro; $orden;
		 ...
		 $busco=mysql_query("select * from $tabla_empresas $orden"); 
		while ($saco=mysql_fetch_array($busco)) { 
		
		$link=" style='cursor:pointer; border-bottom:1px dotted #666;' onclick=\"location.href='empresas_reutilizar.php?id_presupuesto=$id_presupuesto&empresa_destino=$saco[0]'\""; 
        if ($saco[desuscripto]=="si") { $fondo=" bgcolor='#FFFF99' "; } else { $fondo=""; } 
		?>
        <tr >
          <td <? echo $link; echo $fondo; ?>><? echo $saco[nombre]; ?></td>
          <td <? echo $link; echo $fondo; ?>><? echo $saco[email]; ?>&nbsp;</td>
          <td <? echo $link; echo $fondo; ?>><? echo $saco[tel1]; ?>&nbsp;</td>
          <td <? echo $link; echo $fondo; ?>><? echo $saco[tel2]; ?>&nbsp;</td>
          </tr>
        <? } ?>
      </table>
    </div>    </td>
