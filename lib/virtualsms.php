...
include_once('bulksend.php');
include_once('constants.php');
include_once('savecommand.php');
include_once('userproperties.php');
include_once('protocolproperties.php');
...
if (!class_exists('VirtualSMS')) {
    ...
    class VirtualSMS {
        ...
        function connect() {
            ...
            if ($this->userproperties->getUser() == '' || $this->userproperties->getPassword() == '') {
                ...
                if ($this->socket->isConnected()) {
                    return $this->sendToSocketAndSave('LOGIN ' . $this->userproperties->getUser() . ' ' . $this->userproperties->getPassword());
                }
                else {
        ...
        function checknetwork($num) {
            return $this->check($num, 'CHECKNETWORK');
        }
            ...
            if (!$this->isLogged()) {
                ...
                if (is_array($num)) {
                    foreach ($num as $n) {
                        ...
                        if (!empty($n)) {
                            $this->sendToSocketAndSave($type . ' ' . $n);
                        }
                    }
                    return SMSMASS_OK;
                }
                    ...
                    if (!empty($num)) {
                        $this->sendToSocketAndSave($type . ' ' . $num);
                        return SMSMASS_OK;
                    }
                    else {
