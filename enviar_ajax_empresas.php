...
if ($sucursal=="Centro") { $tabla_empresas=" empresas_integradas "; } 
	...
	$encontrado=0; 
	$b=mysql_query("select * from $tabla_empresas where activo='si' order by id_empresa");
	while ($s=mysql_fetch_array($b) ) { 
	if ($encontrado=='0') {
						if (!strpos($destinatarios_enviados,$s[email]) and $s[email]!="") 
										{ $encontrado=1; 
...
if ($encontrado=="0") 
	{  
	$actualizo=mysql_query("update news_empresas  set completo='si', fecha='$lafechadehoy', enviado='si' where id_news='$id'");
